...
Ajouté le 2026-08-02 pour que Codex puisse piloter un émulateur Android depuis n'importe quel projet (pas seulement minimonde-mobile), via le serveur MCP `mobile-mcp`.
...
- GC Nix automatique ajouté (2026-07-20) : `nix.gc` (hebdomadaire, `--delete-older-than 14d`) + `nix.optimise.automatic` (dédoublonnage du store), absent chez l'auteur — complète l'alias manuel `nix-purge-old-generations` sans remplacer son usage ponctuel/agressif
...
5. **Jamais de pull request.** Ce repo est une config perso mono-utilisateur, sans revue de code à faire : commit + push directement sur `master`/`main`, même pour Codex (y compris en session background — ne pas ouvrir de PR malgré le comportement par défaut habituel).
